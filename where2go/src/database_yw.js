...
// Fetch user's Private Space (saved places)
export const getPrivatePlaces = async (userId) => {
  try {
    const placesRef = collection(db, "privateSpaces", userId, "places");
    const snapshot = await getDocs(placesRef);
    const places = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
    console.log("Retrieved private places:", places);
    return places;
  } catch (e) {
    console.error("Error fetching private places:", e);
  }
};
...
// Add a new place to Private Space
export const addPlaceToPrivate = async (userId, placeData) => {
  try {
    const docRef = await addDoc(collection(db, "privateSpaces", userId, "places"), placeData);
    console.log("Place added to Private Space with ID: ", docRef.id);
  } catch (e) {
    console.error("Error adding place to Private Space:", e);
  }
};
...
// --------------------------
// Update the data functions:
// --------------------------

// Update the place cards
export const updatePlaceInPrivate = async (userId, placeId, updatedData) => {
    try {
      const placeRef = doc(db, "privateSpaces", userId, "places", placeId);
      await updateDoc(placeRef, updatedData);
      console.log("Place updated successfully in Private Space");
    } catch (e) {
      console.error("Error updating place in Private Space:", e);
    }
  };
